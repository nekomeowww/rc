...
Worktree must already be Ready, have a child PVC, and have no incompatible
write Lease. A writable mount persists its topology intent and reserves the
Workspace's write Lease before stopping processes; a failed stop rolls both
back. The writable Worktree generated by `workspace mount repo` uses an ordinary
bootstrap Job and reaches Ready before the existing runtime is stopped;
Temporary Workspaces may still use deferred in-runtime checkout because they do
not disrupt an existing machine.

`worktree exec` creates a `WorktreeExec` and runs the exact argv in a Job that
mounts the Worktree child PVC directly. It shares the exclusive Worktree write
Lease with Workspace runtimes. This is intentionally a narrow, lightweight
...
every observation also restores its missing write Lease or stops the Job if
another writer owns the Lease or the Lease cannot be verified. `worktree delete` (also
`remove` and `rm`) first acquires that same Lease and is finalized server-side
...
and Environment selection; namespace resolution; explicit Temporary Workspace creation;
