...
func TestRunCommandStopsParsingRcctlFlagsAtCommand(t *testing.T) {
	...
	require.NoError(t, command.ParseFlags([]string{"--workspace", "dev", testAgentCommand, "--dangerously-bypass-approvals-and-sandbox", testTaskArgument}))
	require.Equal(t, "dev", command.Flag("workspace").Value.String())
...
func TestRunCommandAcceptsOptionalSeparator(t *testing.T) {
	command := newRunCommand(kubeconfig.NewFlags(), true)
	require.NoError(t, command.ParseFlags([]string{"--workspace", "dev", "--", testAgentCommand, testTaskArgument}))
